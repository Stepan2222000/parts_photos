...
- Финальный кадр — одна фотореалистичная картинка. Размер и соотношение сторон результата задаются извне — не дорисовывай поля и не меняй кадрирование произвольно.
"""
# fmt: on
...
# fmt: off
REFINE_LOCKDOWN = """\
Ты дорабатываешь уже сгенерированное фото товара (запчасти) для объявления на маркетплейсе.
Это однопроходная точечная правка: примени все замечания пользователя за один шаг, не предлагай итераций.

Жёсткие правила доработки:
- Изображение 1 — текущая версия фото. Она почти готова: сохрани её целиком — запчасть, геометрию, пропорции, материал, цвет, фон, освещение, тени, кадрирование и композицию — ровно как есть.
- Измени ТОЛЬКО то, что явно просит пользователь в замечаниях ниже. Всё, что в замечаниях не упомянуто, не трогай вообще.
- Сохрани все заводские маркировки: SKU, штрихкоды, номера моделей, серийные номера, lot-коды и любой печатный текст — и по расположению, и по визуальному содержанию. Не выдумывай и не перефразируй ни одного символа.
- Не добавляй новых объектов, сценических элементов, надписей или вотермарок, которых нет на текущей версии, если пользователь явно не попросил.
- Финальный кадр — одна фотореалистичная картинка. Размер и соотношение сторон результата задаются извне — не дорисовывай поля и не меняй кадрирование произвольно.
"""
# fmt: on

_REFINE_REFERENCE_NOTE = (
    "Изображение 2: референс от пользователя — иллюстрация проблемы или желаемого результата "
    "(например, скриншот с пометками или пример, как должно выглядеть). Используй его как указание "
    "к правке. НЕ вставляй содержимое референса в кадр буквально и не копируй его стиль целиком, "
    "если пользователь явно не попросил."
)


def build_refine_prompt(user_prompt: str, *, has_reference: bool = False) -> str:
    """Собирает промпт для доработки уже сгенерированного результата.

    В отличие от build_prompt здесь нет матрицы опций: базовый локдаун
    «сохрани всё как есть», замечания пользователя с приоритетом и финальная
    подсказка по тексту/маркировкам.
    """
    text = (user_prompt or "").strip()
    if not text:
        raise ValueError("refine prompt must be a non-empty string")

    legend = ["Изображение 1: текущая версия фото товара — объект доработки."]
    if has_reference:
        legend.append(_REFINE_REFERENCE_NOTE)

    sections = [
        "\n".join(legend),
        REFINE_LOCKDOWN.strip(),
        "ЗАМЕЧАНИЯ ПОЛЬЗОВАТЕЛЯ (что именно исправить; имеют приоритет над правилами выше):\n" + text,
        _FINAL_LOCKDOWN,
    ]
    return "\n\n".join(sections)
